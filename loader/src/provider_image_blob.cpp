...
image_blob::image_blob(nlohmann::json js) :
    ...
    blob_extractor(blob_config),
    blob_loader(blob_config)
{
    num_inputs = 2;
    oshapes.push_back(image_config.get_shape_type());
    oshapes.push_back(blob_config.get_shape_type());
}

void image_blob::provide(int idx, buffer_in_array& in_buf, buffer_out_array& out_buf)
{
    ...
    // Process target data
    auto blob_dec = blob_extractor.extract(target_in.data(), target_in.size());
    blob_loader.load({target_out}, blob_dec);
}
